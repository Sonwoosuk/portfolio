...
- 섹션들의 `pin`이 transform 방식으로 정상 동작하려면 ScrollSmoother가 먼저 생성되어야 해서, 스무더 준비 완료 후에 섹션을 마운트하는 순서로 처리했습니다.

**2. Matter.js 물리 낙하 → 포트폴리오 표 조립**

- `PortfolioChart.jsx`에서 각 프로젝트 제목의 실제 렌더링 위치를 측정해 물리 바디의 목표 좌표로 사용하고, 낙하·충돌 후 원래 표 위치로 정렬되는 연출을 구현했습니다.
- 모바일에서는 물리 연출 없이 완성된 표를 바로 노출해 성능·터치 조작 문제를 피했습니다.

**3. 작품 상세 → 목록 복귀 시 스크롤 위치 정밀 복원**

```javascript
// src/pages/Landing.jsx — fromWork 복귀 로직
...
- 작품 세부 페이지에서 "목록으로" 돌아오면, 포트폴리오 표 핀 구간 중 표가 이미 완성된 지점(진행도 50%)으로 바로 이동시켜 물리 애니메이션을 다시 보지 않도록 했습니다. 표의 ScrollTrigger가 폰트 로딩 이후 생성되는 타이밍 문제는 최대 20회 재시도로 처리했습니다.

**4. 터치 기기 대응**

- `window.matchMedia('(hover: none), (pointer: coarse)')`로 터치 기기를 감지해 hover 의존 인터랙션(매니페스토, 가로 스크롤 웍스, 포트폴리오 표)을 첫 탭 = hover, 두 번째 탭 = 이동 방식으로 전환했습니다.
- 커서가 없어 성립하지 않는 Playground(이미지 트레일) 섹션은 모바일에서 메뉴 항목째 제외했습니다.
