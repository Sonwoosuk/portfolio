...
void main() {
  ...
  vec2 coord = (fc - iResolution.xy * 0.5) * scale / iZoom;
  vec2 mouseW = (iMouse.xy - iResolution.xy * 0.5) * scale / iZoom;
  float m1 = 0.0;
  for (int i = 0; i < 50; i++) {
  ...
  if (iInvert) {
    m2 *= smoothstep(0.15, 0.6, m1);
  }
  float total = m1 + m2;
  float f;
  if (iFeather > 0.0) {
    f = smoothstep(1.3 - iFeather, 1.3 + iFeather, total);
  } else {
    f = smoothstep(-1.0, 1.0, (total - 1.3) / min(1.0, fwidth(total)));
  }
  if (iInvert) {
    outColor = vec4(iColor, 1.0 - f);
  } else {
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        iZoom: { value: 1 },
        iInvert: { value: inverted },
        iFeather: { value: feather }
      }
    });
    ...
    function update(t) {
      ...
      if (zoomRef) {
        program.uniforms.iZoom.value = zoomRef.current || 1;
      }
